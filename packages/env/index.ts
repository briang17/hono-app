...
const globalEnv = validate(
	globalEnvScope.name,
	globalEnvScope.schema,
	envSource,
);
const apiEnv = validate(apiEnvScope.name, apiEnvScope.schema, envSource);
const databaseEnv = validate(
	databaseEnvScope.name,
	databaseEnvScope.schema,
	envSource,
);
const infisicalEnv = validate(
	infisicalEnvScope.name,
	infisicalEnvScope.schema,
	envSource,
);
...
function validateSecrets<T>(
	secretsScope: SecretScope<T>,
	groupedSecrets: Secret[],
) {
	const parsedSecrets = Object.assign(
		{},
		...groupedSecrets.map((secret) => {
	...
	if (!result.success) {
		throw new Error(
			`Secret validation error: ${secretsScope.name} environment. ${result.error.message}`,
		);
	}
	return result.data;
}
...
async function loadSecrets() {
	...
	const groupedAuthSecrets = secretsSource.filter(
		(secret) => secret.secretPath === authSecretsScope.path,
	);
